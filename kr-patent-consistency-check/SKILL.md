...
A~J 10개 영역. A~G는 한국 특허 일반 점검, H·I는 프롬프트 2(8+3 룰) + 프롬프트 1(S1~S14 정형)과 직접 연동, **J는 상류 Stage(2·3·4) 산출물 역추적**.
...
> **⚠ 도면 OCR은 점검 default 단계**: 도면 파일이 가용한 경우(외부 도면 폴더 또는 docx 임베드) 무조건 OCR을 수행한다. "가능 시" 옵션이 아님. 도면 객체 누락·강조 표기 본문 미반영 같은 결함은 OCR 없이는 검출 불가능하며, P-2026-011-01-KR 사건에서 v12·v15·v18 라운드 3회의 결함이 모두 OCR default였다면 한 번에 검출됐을 사례로 확인됨.
...
- (2026-05-27, P-2026-011-01-KR) '단계 SXXX' / '청구항 N' 자동 grep 시 【부호의 설명】 섹션은 반드시 점검 영역에서 제외. 매핑 표("S110: 설계 정보 수신 단계 / S120: …") 행이 정규식 `단계\s*S\d+`에 매칭되어 false positive 13건 발생한 사례.
- (2026-05-27, P-2026-011-01-KR) 도면 도입 정형 검증식에 한국어 조사 "은/는" 모두 매칭(`[은는]`). 단독 도입 `도\s*N\s*[은는][^.]+이다\.` + 묶음 도입 `도\s*N\s*[은는][^,]+이고,` 두 패턴 동시 검색. 어느 한 패턴만 매칭 시 도 2·4 같은 묶음 도입 도면이 false negative로 잡힘.
- (2026-05-27, P-2026-011-01-KR) 등급별 카테고리 부호(예: A등급 정밀 분리해체 패널 = 162)가 그 카테고리의 대표 장비(타워 크레인)에 부착되어 "타워 크레인(162)" 형태로 등장하는 패턴은 정합성 결함 아님 — 한국 특허 실무상 허용 표현. 등재명과 본문 명칭의 단순 1:1 비교는 false positive 발생.
- (2026-05-27, P-2026-011-01-KR) 새 점검 항목 — 청구항 본문 1:1 매핑 검증: 청구항 1~N의 한정 시그니처(예: 청구항 9의 "인접한 자재끼리 하나의 해체 단위로 결합")가 본문에 한 번 이상 그대로 등장하는지 자동 grep. 누락 시 Priority B 결함. B섹션 [B-5]로 점검 룰 추가 — 지원요건(특허법 §42②) 보강 차원.
- (2026-05-27, P-2026-011-01-KR) 시스템 청구항 자동 grep 시그니처: 청구항 14 같은 시스템 청구항은 방법 청구항과 다른 키워드("메모리; 및
...
- (2026-05-28, P-2026-171-01-KR) 정합성 점검 7항목(금지어/본문 청구항 직접 언급/단계 SXXX 본문 언급/부호 트리 본문 인용 누락/S4 청구항 전수 매핑 게이트/도면 본문 인용 누락/도입어 변주·한정 부정·효과 단락 3단)을 Python 정규식 1회로 검증하는 자동화 패턴 확립 — Priority B 결함 자동 검출 → 사용자 승인 후 자동 교체 → 재검증 0건의 ralph-loop 사이클 검증.
- (2026-05-28, P-2026-171-01-KR) 종결 어미 가능형 정규식은 '할 수 있다'만이 아니라 '될 수 있다·명명될 수 있다·보유될 수 있다·변환될 수 있다·이루어질 수 있다·구현될 수 있다·도출될 수 있다·포함될 수 있다·갱신될 수 있다' 등 가능형 종결 어미 패턴 전체 포함하도록 확장 필요. 좁은 정규식으로 인한 과소 추정 방지(P-2026-171-01-KR 사례: 27.8%로 잘못 산출).
...
- (2026-05-28) 도면 부호가 부착된 객체는 후속 인용에서도 '상기' 붙이지 말 것 — 자유 서술·정의 후속·변형 실시예 모두 동일 적용. 점검 방식: 본문에서 한 번이라도 'X(NNN)' 형태로 부호 부착된 명사 리스트를 만든 후, S4 청구항 풀어쓰기 단락 외에서 '상기 X' 패턴이 등장하면 모두 '상기 ' 제거. 단 'X'가 한 번도 부호 부착된 적 없는 추상 개념(예: '상기 유사도', '상기 제1 단계')은 명확성 위해 허용. 핵심 명사군 예시 (P-2026-011-01-KR): 현장 스캔 데이터·디지털 트윈 모델·객체 탐지 모델·인스턴스 분할 모델·분류 모델·폐기물 처리 비용 정보·자재 판매 수익 정보. 이전 정책 '청구항 복붙한 곳에는 상기 나둬'와 충돌 시 청구항 풀어쓰기(S4) 단락에서만 상기 보존, 그 외는 부호 룰 우선 적용.
