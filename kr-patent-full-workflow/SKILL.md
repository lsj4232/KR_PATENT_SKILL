...
2. **기술분야 프로파일 판정 (★ 필수, 다른 판단보다 먼저)**
   ...
   | 프로파일 | 판별 신호 | 하류 영향 |
   |---|---|---|
   | **G** (AI/SW/제어/시스템) | 청구항이 방법(단계 SXXX), 시스템, 기록매체항. 도면이 흐름도, 블록도, 개념도 | 기존 Stage 3~10 그대로. 제1독립항은 방법항 |
   | **H** (기계·전기전자·화학) | 청구항이 물리적 구성의 결합. 도면이 사시도, 단면도, 분리도, 평면도 | Stage 4·5는 `hw-mechanical-spec-rules.md` 델타 적용, Stage 8은 L섹션 분기. **제1독립항은 물건항**(`kr-patent-claim1-drafting` P-0, P-0-2) |
   | 혼합 | 장치항 + 제어 방법항 병존 | H 기본 + 제어 방법항만 G 정형 병용. **제1독립항은 물건항**, 제어 방법항은 제2독립항 |

   H로 판정되면 **유형 A(단일 실시형태) / 유형 B(복수 실시형태)까지 이 시점에 확정**한다.
   유형이 뒤집히면 S4, S5, S13 전체를 다시 써야 하므로 Stage 4 진입 전 사용자 확인 필수.
...
**카테고리 정합 체크 (★ Stage 0 프로파일 연동)**: Stage 0에서 H 또는 혼합으로 판정되었는데 제1항이
방법항이면, 청구항이 아직 확정 전(사내 초안, 미출원)인 경우에 한해 물건항 전환을 1회 제안한다.

> "기구 발명인데 제1항이 방법항입니다. 물건항으로 세우면 침해 입증이 피고 제품 구조 대비만으로
> 끝나고, 유통 단계까지 권리가 미칩니다. 제1항을 물건항으로 재구성하고 현재 방법항을
> 제2독립항으로 내릴까요? (근거가 공정 진보성에 있다면 현행 유지)"

승인하면 `kr-patent-claim1-drafting` P-0-2로 물건항을 작성하고, 방법항은
`kr-patent-dependent-claim-drafting`으로 제2독립항 처리한다. 이미 출원된 건, 또는 사용자가 현행
유지를 택하면 원문 그대로 진행한다. 어떤 경우에도 임의 수정은 금지.
