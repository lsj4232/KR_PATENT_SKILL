...
2. **기술분야 프로파일 판정 (★ 필수, 다른 판단보다 먼저)**
   ...
   | 프로파일 | 판별 신호 | 하류 영향 |
   |---|---|---|
   | **G** (AI/SW/제어/시스템) | 청구항이 방법(단계 SXXX), 시스템, 기록매체항. 도면이 흐름도, 블록도, 개념도 | 기존 Stage 3~10 그대로 |
   | **H** (기계·전기전자·화학) | 청구항이 물리적 구성의 결합. 도면이 사시도, 단면도, 분리도, 평면도 | Stage 4·5는 `hw-mechanical-spec-rules.md` 델타 적용, Stage 8은 L섹션 분기 |
   | 혼합 | 장치항 + 제어 방법항 병존 | H 기본 + 제어 방법항만 G 정형 병용 |
