...
| **Stage 7 (실시예 추가)** | 16카테고리 중 어떤 카테고리를 어느 절에 적용할지 변리사 판단 필요 (절 성격 적합성·청구항 충돌 회피). |
...
이 시점의 산출물은 **1차 초안** — 권리범위 보강(Stage 6·7)이 추가되어야 비로소 실무 수준 명세서가 됨.
...
- 16카테고리 중 절의 성격에 자연스러운 4~7개 카테고리 추천 (절×카테고리 매트릭스 작성)
...
| 7 | 추가된 변형 실시예 단락 + 절×카테고리 매트릭스 | content.js (본문 통합) |
...
### "X 단계 건너뛰어줘"
→ Stage 1, 4, 6, 7, 9, 11은 조건에 따라 건너뛸 수 있음. Stage 2, 3, 5, 8, 10은 핵심이라 건너뛰면 경고. 특히 Stage 6·7(권리범위 보강)을 건너뛸 경우 "권리범위가 좁아질 수 있습니다. 정말 건너뛰시겠습니까?" 명시 경고.
