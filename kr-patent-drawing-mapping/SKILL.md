...
도면 박스 명칭이 본문과 불일치하면 도면 오타. 예시 (사건 A 사건):
...
- **kr-patent-drawing-tagging**: 매핑 표를 도면 PNG에 **실제로 그려 넣어** 태깅본을 만들려면 호출 (cv2 박스검출 + 충돌감지 배치, 본 스킬 매핑표가 config의 원천).
...
- (2026-05-28, 사건 D) 청구항 docx 한 파일에 청구항 텍스트와 도면(TIF·JPEG·PNG 혼재)이 임베드된 케이스 — zipfile 모듈로 word/media 디렉토리 추출하여 도면 이미지 자동 분리. TIF는 PIL/Pillow로 PNG로 변환 후 시각 분석.
- (2026-05-28, 사건 D) 도면 분류 default = 전수 사용. 한국어/영문/실험 그래프 혼재 시 영문 도면이라도 부호가 청구항·한국어 도면과 정합되면 명세서에 그대로 사용 가능. 자동 배제 금지 — 변리사 결정 권한 사항으로 분류만 제안하고 사용 여부 결정 입력 받기.
