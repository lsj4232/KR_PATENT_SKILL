...
- (2026-05-28, P-2026-171-01-KR) 청구항 docx 한 파일에 청구항 텍스트와 도면(TIF·JPEG·PNG 혼재)이 임베드된 케이스 — zipfile 모듈로 word/media 디렉토리 추출하여 도면 이미지 자동 분리. TIF는 PIL/Pillow로 PNG로 변환 후 시각 분석.
- (2026-05-28, P-2026-171-01-KR) 도면 분류 default = 전수 사용. 한국어/영문/실험 그래프 혼재 시 영문 도면이라도 부호가 청구항·한국어 도면과 정합되면 명세서에 그대로 사용 가능. 자동 배제 금지 — 변리사 결정 권한 사항으로 분류만 제안하고 사용 여부 결정 입력 받기.
