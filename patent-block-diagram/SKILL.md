...
| 판단·분기 | `shape: "diamond"` |
| 네트워크(망) | `shape: "cloud"` (음영 없음. 글자는 구름 몸통에 맞춰 조금 아래에 놓인다). **cloud는 "망"을 뜻하므로 외부 시스템에 함부로 쓰지 않는다.** 언어 모델처럼 시스템의 통신 상대인 별개 객체는 `rect`가 맞다 |
...
**`dx`, `dy`는 숫자의 왼쪽 끝이 아니라 가운데가 놓이는 좌표다.** 이것을 모르면 `dx = w + 40`에서
숫자의 왼쪽 절반이 박스 안으로 파고든다(fs 56에서 세 자리 숫자 반폭 약 85px).

- **기본은 박스 위쪽** `dx = w/2` 또는 `40`, `dy = -70 ~ -75`. 화살표는 박스 중심 높이로 들어오고
  팬인·팬아웃 스파인은 좌우 바깥을 지나므로 위쪽 좌측 모서리는 거의 항상 비어 있다.
- **오른쪽 바깥은 `dx = w + 140`**, 그리고 **그 행에서 가장 오른쪽 박스에만** 쓴다. 체인 중간
  박스에 달면 숫자가 다음 박스 좌상단에 올라앉는다(박스 간격이 보통 120~160px).
- 캔버스 우측 끝 박스는 왼쪽(`dx = -125`) 또는 위(`dy = -50`)로 보낸다. 좌측 끝 박스에 음수
  `dx`를 쓰면 좌표가 음수가 되므로 저장 시 우측 시프트 가드를 둔다.
- 흐름도 단계부호(S계열)는 박스 **왼쪽** `dx ≈ -62`. 컨테이너 부호는 위쪽 바깥 `dx = 60, dy = -45`.
- 세로로 쌓은 박스에 위쪽 태그를 쓰면 박스 간 세로 간격을 `h + 100` 이상 잡는다(80에서 물린다).
  점선 컨테이너는 내부 박스 묶음보다 위아래 각 120px 이상 크게 잡는다.
- **화살표가 드나드는 변에는 태그를 두지 않는다.** 태그 자동 배치는 박스 충돌만 보고 화살표
  경로는 보지 않으므로, 렌더 후 "숫자가 선 위에 앉았는가"를 육안으로 따로 본다.
- 부호 체계는 기존 관행대로: 시스템 100 → 하위 110, 120 / 계층은 300 → 310, 320.

전체 규약(인쇄 검산식, z 규약, 절대 금지 4항, 채움 의미 코드, 레이아웃 함정)은
`references/도면_스타일가이드_템플릿.md` §1.
...
3. **.pbd 작성**: 사건 폴더에 `도면N_이름.pbd`로 저장. z는 **컨테이너=1~8, 화살표=15, 박스=20,
   태그=60**. 컨테이너 rect는 흰색 채움이 있어 z가 더 작은 화살표를 완전히 가리므로, 화살표가
   컨테이너보다 위이고 박스보다 아래여야 한다.
...
- (2026-09-16 정리, 도면 13장 세트 실측) **★ 스타일가이드 템플릿 §1 전면 갱신.** 구버전 fs
  16~24는 인쇄 0.8~1.2mm로 규정 위반이라 폐기하고, `fs >= 0.0188 x W`(폭) 또는 `0.01255 x H`(높이)
  중 큰 값, 실무 확정값 **fs 44 / 캔버스 폭 2300 / 높이 3400**으로 교체했다. 사무소 기출원 실측
  목표는 `fs / W ≈ 0.03`. fs 44에서 폭 2300px에 들어가는 열은 **최대 3열**이므로 이 제약이
  레이아웃을 규정한다.
- (2026-09-16) **컨테이너가 화살표를 삼킨다.** 컨테이너 z=1~8, **화살표 z=15**, 박스 20, 태그 60.
  컨테이너 안 블록들을 잇는 화살표가 통째로 사라진 렌더가 실제로 나왔다.
- (2026-09-16) **세로 팬인은 반드시 관통한다.** 같은 x 열에 쌓은 N개 박스를 아래 한 박스로
  모으면 첫 박스에서 나온 선이 중간 박스들을 뚫는다. `bend` 우회로 해결되지 않고 **가로 3열
  재배치**가 유일한 해법이다. 팬아웃(1 -> N)은 세로 배치가 정상이다.
- (2026-09-16) **태그 dx, dy는 중앙 정렬 좌표다.** 우측 배치는 `w + 140`, 그것도 그 행의 맨
  오른쪽 박스에만. 체인 중간 박스의 우측 태그는 다음 박스 좌상단에 올라앉는다. **기본값을
  위쪽 배치(`dx = w/2, dy = -70`)로 바꾸면 스파인 충돌이 한 번에 사라진다.**
- (2026-09-16) **★ 흐름도에는 단계 박스 옆에 산출물 객체 박스를 세운다.** 절대 금지 2항 때문에
  흐름도만 그리면 그 단계의 산출물 부호가 도면에 못 올라가고, 본문은 그 부호를 서술하므로
  정합성 점검에서 「본문에만 있는 부호」로 걸린다. 세로 스파인에 단계 박스를 쌓고 각 단계
  오른쪽에 산출물 객체 박스를 중심 y 맞춰 세운다(단계 박스는 S부호만, 객체 박스는 객체 부호만).
  가로로 넓어지므로 인쇄 검산을 다시 돌린다. **행위 주체(프로세서, 시스템)는 예외**로 흐름도에
  그리지 않으며 정합성 점검에서 화이트리스트로 뺀다.
- (2026-09-16) **검증은 OCR보다 `.pbd` 직접 대조가 정확하다.** `boxes[].text`와 `tags[].label`을
  읽어 본문과 대조하면 100% 정확하다. 이 방법으로 "도면에 있으나 본문에 한 번도 서술되지 않은
  라벨" 10건을 찾았다. 반대 방향(본문 부호 - 도면 태그 차집합)이 더 위험하며, 청구항 구성요소가
  미도시면 지원요건 시비가 붙는다.
- (2026-09-16) **자동 검산에서 빈 컨테이너의 fs를 잡지 마라.** `text: ""` 컨테이너는 fs 기본값이
  15라 「모든 박스 fs >= 52」 규칙에 전부 FAIL로 잡힌다. fs 검사와 `minfs` 계산 모두
  `text.strip()`이 있는 박스만 대상으로 하고 `shape == "image"`도 제외한다.
- (2026-09-16) **도면 라벨 문구는 본문 표기에 맞춘다.** 도면은 명사형("의미적 대응 판단"),
  본문은 동사형("의미적 대응을 판단하는 단계")으로 갈리는데, 심사관과 번역자가 둘을 대조하므로
  도면 쪽을 고치는 것이 옳다. 조사 하나 차이도 맞춘다.
- (2026-09-16) **육안 검증은 3회로 안 끝난다.** 13장 세트에서 7회전이 걸렸고 회전마다 결함
  유형이 달랐다(z 은닉 -> 부호 잘림 -> 경로 관통 -> 폰트 규격 -> 재배치 후 신규 관통 -> 태그
  여유 -> 최종 미세). 중간 회전은 scale 0.7~1.2로 돌려도 결함이 보인다. 최종만 scale 3.
