...
## [S0] 기술분야 프로파일 판별 (분기 게이트) ★ S1보다 먼저

이 스킬의 S1~S7 본문은 **G 프로파일(AI, SW, 제어, 시스템)을 기본 전제**로 쓰여 있다.
기계, 전기전자, 화학 발명은 정형 문구와 어미 규칙 자체가 다르므로, **어떤 섹션도
쓰기 전에** 프로파일을 확정한다.

| 프로파일 | 대상 | 판별 신호 | 적용 규범 |
|---|---|---|---|
| **G** | AI/ML, 데이터 처리 SW, 제어, 시스템/네트워크, BM | 청구항이 방법(단계 S110~), 시스템, 기록매체항 구조. 도면이 흐름도, 블록도, 개념도 | SKILL.md 본문 + `references/sw-ai-spec-supplement.md` |
| **H** | 기계, 전기전자(하드웨어), 화학 | 청구항이 물리적 구성의 결합. 도면이 사시도, 단면도, 분리도, 평면도 | SKILL.md 본문 + **`references/hw-mechanical-spec-rules.md` (델타 우선)** |
| **혼합** | 기구 + 제어 로직 병존 | 장치항 + 제어 방법항이 함께 존재 | H를 기본으로 하되, 제어 방법항 부분은 G 규칙(S13 도면 단락 정형, S14 2파트)을 병용 |

**H 프로파일이면 반드시 `references/hw-mechanical-spec-rules.md`를 먼저 읽고 착수한다.**
주요 델타 3가지만 미리 명시한다.

1. **어미**: 독립항 대응 구성은 **확정형("~한다", "~형성된다")**. 종속항과 실시예만 "~할 수 있다".
   (G 프로파일의 "신규 문장 전부 ~할 수 있다" 규칙은 H에서 적용하지 않는다)
2. **실시형태 수 판별**: 유형 A(단일) / 유형 B(복수)를 먼저 확정. 유형 B는 S4, S5, S7, S13
   전체가 실시형태별 반복 구조가 된다.
3. **도면 정형**: "도 1은 ~의 사시도이다" 형태로 도면 종류를 그대로 쓴다
   (G의 "~을 설명하기 위한 개념도이다" 정형과 다름). 각 도면 섹션은 마무리 효과 문단으로 닫는다.

**G 프로파일이면** `references/sw-ai-spec-supplement.md`의 볼륨 강제(V1~V7),
실시예 4계층(L1~L4), 챕터 목차 골격, 유형별(G1~G4) 필수 서술 항목, 승인 게이트를 함께 적용한다.
...
**★ [추가 1] 카테고리 순서는 청구항 순서와 일치시킨다.** 방법 독립항이 제1항이면
명칭에서도 방법을 앞에 둔다.
...
**★ [추가 2] 말미 앞부분이 두 카테고리에서 동일하면 하나로 묶는다.** 청구항 말미가
"○○ 제어 방법"과 "○○ 제어 시스템"으로 앞부분이 같으면 "이의 ~ 방법" 형태를 쓰지
않는다. 말미 앞부분이 서로 다르면 "및 이의" 형태를 유지한다.

```
(권장) ○○ 제어 방법 및 시스템
(장황) ○○ 제어 시스템 및 이의 제어 방법
```

**★ [추가 3] 명칭에 판정 방식이나 진보성 축을 노출하지 않는다.** 기술내용설명서의
제목을 그대로 명칭으로 쓰지 않는다. 발명의 핵심 수단이 명칭에 드러나면 서지사항만으로
발명의 요지가 공개된다.

```
(노출) 비공개 의존성 및 설계계보 출처에 기초한 ○○ 근거 완전성 판정과
       문서 생성 제어 방법 및 시스템
(권장) ○○ 문서 생성 제어 방법 및 시스템
```

**★ [추가 4] 종속항으로 내려간 구성은 명칭에 반영하지 않는다.** 청구항 설계 단계에서
독립항에서 빠진 축이 명칭에 남아 있으면 명칭과 청구항의 정합이 깨진다.
...
청구항의 말미(「를 포함하는 X.」 또는 「~인 것을 특징으로 하는 X.」)는 해결 수단으로
옮길 때 어미만 「할 수 있다」로 바꾸고, 말미의 청구 대상 표현은 문장 구조에 맞게
정리한다.
...
(청구항)   ~하는 것을 특징으로 하는 ○○ 제어 시스템.
(해결 수단) ~할 수 있다.
```

### [원칙 10] ★ 종속항 이관 시 주어 보충

「더 포함하는」 형태의 종속항은 청구항에서 인용항이 주어를 공급하지만, 해결 수단에서는
문장이 독립하므로 **주어를 보충해야 한다.**

```
(청구항) 제1항에 있어서, [추가 구성]하는 단계를 더 포함하는 것을 특징으로 하는
         ○○ 제어 방법.
(잘못)   본 발명의 일 실시예에 있어서, [추가 구성]하는 단계를 더 포함할 수 있다.
(권장)   본 발명의 일 실시예에 있어서, 상기 ○○ 제어 방법은 [추가 구성]하는 단계를
         더 포함할 수 있다.
```

「~는 ~인 것을 특징으로 하는」 형태의 한정형 종속항은 이미 주어가 있으므로 보충하지
않는다.

### [원칙 11] ★ 저장매체 문단 (해외출원 대비 건)

프로그램 청구항을 두고 미국 또는 PCT 진입 가능성이 있는 건에서는, **프로그램 독립항
문단 바로 뒤에 비일시적 컴퓨터 판독가능 저장 매체 문단을 배치**한다. 기재근거는 최초
명세서에만 넣을 수 있으므로 후속 리비전으로 미루지 않는다.

정형 문구와 배치 규칙, S14 0파트와의 역할 분담, 맺음말 2층 정의는
**`references/overseas-crm-support.md`** 참조.
...
### ★ 청구항과의 정합 (효과란의 두 가지 함정)

**(1) 청구하지 않은 처리를 효과로 주장하지 않는다.**

효과란이 청구항보다 넓어지면 심사관이 그 효과의 근거가 되는 처리를 청구항에서 찾지
못한다. 각 효과가 어느 청구항의 어느 처리에서 도출되는지를 대응표로 확인한다.

```
(청구항) 근거를 재판정하고, 갱신 결과에 따라 산출물을 회수하는 단계
(잘못)   "재생성의 범위가 한정될 수 있다"
         (재생성은 청구하지 않았음. 회수까지만 청구함)
(권장)   "변경의 영향을 받는 범위에 대하여 선택적으로 재판정할 수 있다"
```

**(2) 서로 다른 청구항에서 도출되는 효과를 한 문단에 섞지 않는다.**

두 종속항의 효과를 한 문단에 묶으면, 상위 종속항만으로 하위 종속항의 효과까지
얻어지는 것처럼 읽힌다.

```
(잘못) 시점 필터링에 의하여 조회 관계 수가 축소되고, 아울러 심사가 종결되지 않은
       자료가 배제될 수 있다.
       (앞은 제N항, 뒤는 제N+2항의 효과. 제N항만으로 종결 판정은 이루어지지 않음)
(권장) 각 청구항의 효과를 별도 문단으로 분리
```

효과란에 담지 못한 하위 청구항의 효과는 S13 챕터별 본문의 기술적 의의 서술로 옮긴다.
...
도면은 명세서의 부속물이 아니라 **명세서 구조의 결정 요인**이다. 한 장이 한 챕터에
대응하고, 도면 순서가 챕터 순서가 되며, 도면 등장 객체 순서가 부재번호 도입 순서가
된다. **종속항의 축 구성이 곧 도면 세트의 설계도**다.

요지 6가지만 본문에 둔다. 전체 규격(세트 골격과 축별 예산, 흐름도 박스 문언 규칙,
선 및 도형의 의미론, 응용 장면 배치, 도면 작업지시서 7구성)은
**`references/drawing-set-design.md`** 참조.

1. **커버리지 우선**: 청구항 축의 도면 뒷받침이 장수에 우선한다. 장수를 이유로 축을
   생략하거나 병합하지 않는다. 각 도면에 대응 청구항을 명시하고, 역으로 도면 뒷받침이
   없는 청구항 목록을 산출한다. 장수 초과 시 초과 사실과 장별 내역을 보고하고 그대로
   진행하며, 압축은 사용자가 명시 선택한 경우에만 수행한다.
2. **한 장 = 한 정보 변환**: 개념 전개 연작은 직전 장의 산출물 부호를 입력으로 받아
   다음 산출물을 만든다. 이 구조가 브릿지 문장과 의미 비약 방지의 근거가 된다.
3. **데이터에서 데이터로 직결 금지**: 변환에는 그 변환을 수행하는 주체 또는 처리 박스를
   경유시킨다.
4. **흐름도 박스 문언**: 방법 독립항 전문 흐름도의 각 박스는 청구항 단계 문언을 그대로
   옮기고, 조정은 「상기」 삭제, 말미 처리, 세미콜론 삭제, 줄바꿈 4가지로 한정한다.
   요약형 박스와 원문형 박스를 한 세트에 섞지 않는다(세트 단위 통일).
5. **종속항은 전문 흐름도에 실선 박스로 섞지 않는다**: 세부 흐름도, 점선 부가, 개념
   전개 도면 중 하나로 처리한다. 섞으면 종속항 한정이 독립항 필수 구성처럼 읽힌다.
6. **도면 용어는 청구항 확정 어휘로 통일**: 발명자료의 모듈명(PIM, NPU 등)을 도면에
   그대로 쓰지 않는다. 도면에서 용어가 어긋나면 본문의 청구항 용어 일관성이 깨진다.

**도면 미확보 건에서는 S11 서두보다 S12를 먼저 수행한다.** 도면이 확정되어야 서두의
시스템 정의와 용어 정리를 부재번호에 맞추어 쓸 수 있다.
...
#### [절대 원칙 5-1] ★ 처리 로직을 `~부`로 쪼개지 않는다

마스터 프롬프트 A-5의 시스템항 규칙이 본문 서술에도 그대로 적용된다.
`~부`로 이름 붙여 개별 구성요소로 세울 수 있는 것은 **물리적 실체가 있는 블록뿐**이다.

- 허용: 입력계(수신부, 스캔부, 센서부, 입출력부), 저장계(메모리, 저장부), 출력계(사용자 인터페이스부, 디스플레이부, 알림부, 통신부), 그리고 **연산 주체 1개**(제어부 또는 프로세서)
- 금지: `~획득부`, `~판단부`, `~탐색부`, `~판정부`, `~검증부`, `~생성부`, `~산출부` 등 **내부 처리 로직의 블록화**

처리 흐름은 `제어부(110)는 ~하고, 이어서 ~하며, 나아가 ~한다` 형태로 **단일 주체의 연속 동작**으로 쓴다.
3관점 서술(원칙 5)의 "하드웨어 구성 예시"는 제어부·메모리·통신부 수준에서 끝내고,
세부 처리는 "구체적 처리 내용" 관점에서 동작으로 풀어쓴다.

⚠️ 이 실수는 **본문을 다 쓴 뒤에 발견하면 수정 비용이 수십 배**다(실측: 도면 1장 재작도 + 본문 58곳 치환).
S13 착수 전에 청구항 시스템 독립항이 부르는 구성요소 이름을 확인하고, 거기 없는 `~부`는 만들지 않는다.
부호 설계 단계 규칙은 [[kr-patent-symbol-design]] §2-1.
...
#### ★ 0파트 (저장매체 구현 문단), 해외출원 대비 건에 한함

프로그램 청구항을 두고 미국 또는 PCT 진입 가능성이 있는 건에서는, 1파트 효과 정리
**앞**에 저장매체 구현 문단을 배치한다. 실시예 서술의 성격이므로 마지막 챕터 뒤에
자연스럽게 이어지며, **부재번호를 붙여** 본문의 다른 서술과 계열을 맞춘다.
...
S4 저장매체 문단(부재번호 없음, 청구항 계열 문언)과 **역할을 나누어 같은 문장을 두 번
반복하지 않는다.** 마지막 문장은 저장 매체가 시스템과 독립적으로 유통되는 물건임을
뒷받침하므로 생략하지 않는다.

또한 2파트 매체 일반론은 **2층 정의**로 구성한다. 상위 용어("컴퓨터 판독 가능한 기록
매체")는 `포함할 수 있다`로 열고, 하위 용어("비일시적 컴퓨터 판독가능 저장 매체")만
`포함되지 아니한다`로 닫는다. 상위까지 닫으면 국내 프로그램항의 범위를 스스로 좁히고,
하위를 열어 두면 미국 심사에서 transitory 배제 근거가 되지 못한다. 정본 단락과 매체
예시 주의(NVMe 금지 등)는 **`references/overseas-crm-support.md`** 참조.
...
5. **2026.09 개정 추가 점검 항목** (해당 건에만 적용):
   - 명칭의 카테고리 순서가 청구항 순서와 일치하는가. 명칭에 판정 방식이나 진보성 축이
     노출되지 않았는가. 종속항으로 내려간 구성이 명칭에 남아 있지 않은가.
   - 「더 포함하는」 종속항을 S4로 옮길 때 주어를 보충했는가.
   - 효과란의 각 효과가 청구항이 실제로 청구한 처리에서 도출되는가. 서로 다른 청구항의
     효과를 한 문단에 섞지 않았는가.
   - (해외출원 대비 건) 저장매체 지지문언 2곳(S4, S14 0파트)이 모두 있고 역할이
     구분되는가. 맺음말 매체 일반론이 2층 정의인가. 매체 예시에 NVMe가 없는가.
     명세서에 특정 국가법 대응 목적을 드러내는 서술이 없는가.
   - 청구항과 도면의 커버리지 매핑을 작성했고, 도면 뒷받침이 없는 청구항 목록을
     파악하고 있는가.
   - 상세한 설명 내에서 같은 내용이 두 챕터에 중복 서술되지 않았는가.
   - 청구항에서 삭제하거나 완화한 한정이 명세서 실시예로 남아 있는가(거절 대응 시 끌어올
     한정 후보).

---

## 부가 작성: 요약서 / 대표도 / 도면의 간단한 설명 / 부호의 설명

### 요약서
- 목표 분량은 청구항 1의 실체를 압축 없이 담는 최소 규모(약 200~350자).
- 청구항 1의 문언을 그대로 가져와 **하나의 문장**으로 구성한다.
- 시스템 청구항과 프로그램 청구항은 방법 청구항의 실체와 겹치므로 별도 서술하지 않고
  **"및 이를 수행하는 [시스템 명칭]"** 문구로 통합한다.
- 프로그램 청구항은 요약에 반영하지 않는 것이 실무 관례다.

### 대표도
- 시스템 전체 구성도 또는 방법 전문 흐름도를 지정한다.
- 심사관과 제3자가 발명의 개요를 빠르게 파악할 수 있는 도면을 고른다.

### 도면의 간단한 설명
- 각 도면의 첫 문장을 **S13 본문의 도면 도입 첫 문장과 글자 단위로 일치**시킨다.
- 부재번호는 도면의 간단한 설명 자체에는 표기하지 않는다(실무 표준).

### 부호의 설명
- **【부호의 설명】 섹션을 두지 않는 것이 default다**(동의어 방어망 보존). 출원인이
  요구하는 건에서만 작성한다.
- 작성하는 경우에도 도면에 표기되는 부호만 기재하고 **단계 부호(S110 등)는 기재하지
  않는다.**
- 내부 관리용으로는 부호별 최초 등장 챕터와 등장 도면을 정리한 표를 별도 산출하여
  명세서와 도면의 정합 확인에 이용한다.

### 산출 파일 목록
명세서 초안 완성 시 함께 산출한다.

- 명세서 본문 docx
- 도면 구성안 및 청구항과 도면의 커버리지 매핑표
- 도면 작업지시서 또는 생성 도구용 프롬프트(도면 미확보 건)
- 부호 정리표
- 청구항과 명세서 정합성 자가 점검표
- 발명자 확인 필요 사항 목록(복수 실시예로 열어 둔 채 구현이 확정되지 않은 항목)
...
## 실시예 4계층 (L1~L4) — 프로파일 공통

각 챕터(H 프로파일은 각 도면 섹션)에서 아래 4계층을 다층으로 쌓는다. L2~L4가
분할출원, 계속출원의 권리범위 베이스가 되므로 **청구항 승격이 가능한 수준**까지 쓴다.

| 계층 | 성격 | G 프로파일 도입 문형 | H 프로파일 도입 문형 |
|---|---|---|---|
| **L1** | 기본 실시예 (핵심 흐름 통합) | "일 실시 형태에 있어서, ~" | 독립항 대응 **확정형** "~한다" |
| **L2** | 변형 실시예 | "다른 예로서, ~할 수 있다." | "일 예로 ~할 수 있다. 다른 일 예로 ~할 수 있다." |
| **L3** | 선택 실시예 (조건부 분기) | "일 실시예에 따르면, ~경우에는 ~할 수 있다." | "이때, ~한 경우 ~할 수 있다." |
| **L4** | 대체 실시예 | "추가적으로 또는 대안적으로, ~할 수 있다." | "~일 수 있다 / ~할 수도 있다" |

H 프로파일은 변형예를 새로 만들기보다 **기존 특징을 다른 관점(구조→기능, 정지→운동,
형태→관계)에서 다시 서술**하는 것을 우선한다.

## 볼륨 하한 (V1~V7) — 지원요건 자가 점검선

| # | 하한 |
|---|---|
| V1 | 실시예 본문 최소 600문장 / 25,000자 / 30페이지 (권장) |
| V2 | L1~L4 4계층 모두 작성 |
| V3 | 【발명의 내용】 각 하위섹션 최소 5문장 |
| V4 | 배경기술 최소 3단락, 각 단락 5문장 (H 프로파일은 4~8문단, 700~1,400자) |
| V5 | 종속항 총 최소 8개 (권장 15~25개) |
| V6 | **사후 보완 문단 삽입 시 반드시 15~25문장** (3문장 이하 금지 — 심사 대응 근거로 못 씀) |
| V7 | 구성요소 1개당 최소 3회 서술 (정의 → 구체화 → 조건/변형) |

세부는 `references/sw-ai-spec-supplement.md` 2~3절.
...
### H 프로파일(기계·전기전자·화학) 전용 — G 항목 대신 적용

위 형식·정형 항목 중 S2 정형 종결, S12 도면 정형, S14 2파트는 H 프로파일에서 적용하지
않는다. 대신 아래를 점검한다. 전수 항목은 `references/hw-mechanical-spec-rules.md` 5절.

- [ ] 발명 유형(A 단일 / B 복수 실시형태)이 확정되고 전 섹션에 일관 적용
- [ ] **독립항 구성은 확정형("~한다"), 종속항·실시예는 "~할 수 있다"**
- [ ] S2 마지막 문단 "이에 따라, ~할 수 있는 [발명의 명칭]의 개발이 요구된다"
- [ ] S2에 종래기술 자인("종래에는 ~하였다") 0건, 문헌 특정 인용 0건
- [ ] S5 효과가 S3 과제와 1:1 대응, 각 효과에 주체(장치명) 명시
- [ ] S12 "도 N은 ~의 [사시도/단면도/분리도]이다" 형식, 유형 B는 실시형태 소속 명시
- [ ] S13 도면 참조가 엄격 순차, 역참조는 (도 N 참조) 괄호 표기
- [ ] **각 도면 섹션이 마무리 효과 문단으로 종료 (마지막 섹션 포함)**
- [ ] 유형 B: 전환 브릿지 + 중복 회피 문구 + 차이점 명시 3종 모두 존재
- [ ] 유형 B: 동일 구성 = 동일 참조번호
- [ ] 영문 병기가 외래어에 한정, 최초 등장 위치 1회만, 청구항 구성 명칭에는 미병기
- [ ] "구현하는", "구성한다/구성된다", "반드시/항상/오직" 0건
- [ ] 마감 문단이 하나의 문단으로 병합
- [ ] 물리적 정합: 위치 관계, 크기·형상 관계, 작동 인과가 도면과 모순 없음
...
### 누적 학습 항목
- (2026-09-16, kr-spec-writing 20260910 통합) 다운로드 통합본과 대조하여 6개 블록 반영: S1 명칭 4규칙(카테고리 순서 청구항 일치, 말미 앞부분 동일 시 묶기, 진보성 축 비노출, 종속항 강등 구성 배제), S4 원칙 9~11(말미 형식 처리, 「더 포함하는」 종속항 주어 보충, 저장매체 문단), S5 효과란 두 함정(청구하지 않은 처리의 효과 주장 금지, 서로 다른 청구항 효과 혼재 금지), S12 도면 세트 설계 6요지, S14 0파트와 2층 정의, 부가 작성(요약서, 대표도, 부호의 설명). ⚠️ **정본 선택으로 남긴 3건은 반영하지 않았다**: (1) 기술분야 "반드시 한 문장" vs 기존 2문장 관행, (2) S14 1파트 도입 문구 "이와 같이, 본 발명에 따른 [명칭]에 의하면" vs 기존 "이상에서 살펴본 바와 같이", (3) 단계 부호 도면번호x100 대역 vs 기존 S110 순차. 건별로 하나를 골라 세트 단위로 통일한다.
- **★ S0 기술분야 프로파일 분기 도입 (2026-08-24)** — 기존 S1~S14 정형은 G 계열(AI/SW/제어/시스템) 전용 전제였다. 기계, 전기전자, 화학 발명에 그대로 적용하면 (1) 독립항 구성까지 "~할 수 있다"로 써서 필수 구성이 선택적 부가로 읽히고, (2) 컴퓨터 판독 매체 정형 문구가 무의미하게 붙고, (3) "~을 설명하기 위한 개념도" 정형이 사시도, 단면도 명세서에 어긋난다. S1 착수 전에 프로파일을 판정하고 H 계열이면 `references/hw-mechanical-spec-rules.md` 델타를 우선 적용한다.
- **H 계열 유형 A/B 판별은 되돌리기 비용이 가장 큰 결정 (2026-08-24)** — 복수 실시형태(유형 B)로 판정되면 S4, S5, S13이 실시형태별 반복 구조가 되고 전환 브릿지, 중복 회피 문구, 차이점 명시 3종이 필수가 된다. 유형이 뒤집히면 사실상 전면 재작성이므로 착수 전 사용자 확인을 받는다.
- **H 계열 마지막 도면 섹션의 마무리 효과 문단 누락이 빈발 (2026-08-24)** — "위에서 설명한 [구성]의 구성에 의하면, ~. 이에 따라, ~할 수 있다." 문단은 각 도면 섹션마다 필요하며 마지막 섹션에서 특히 잘 빠진다. consistency-check L-2-7로 자동 검출한다.
- **볼륨 하한 V6(보완 문단 15~25문장) (2026-08-24)** — 권리범위 보강 단계에서 3문장짜리 문단을 끼워 넣으면 심사 대응 근거로 쓰이지 못한다. 보완은 항상 문단 단위 실체 서술로 한다.
...
- (2026-08-06, 사건 F) **★★★ 청구항에 없는 `~부` 신설 금지 (기능부 남발 금지)** — 장치·시스템 실시예(S13)와 도 1 블록도에서, 청구항에 기재된 블록(통상 `메모리`, `제어부`, 그리고 실체가 있는 입력계·저장계·출력계)만 구성요소로 두고 **내부 처리 로직을 `문서 구조화부`, `트리플 추출부`, `증거 가중치 산출부` 식으로 쪼개 부호를 붙이지 않는다**. 근거는 마스터 프롬프트(`<사건 폴더 루트>/특허_마스터_프롬프트.md`) A-5: "변환·판정·산출 등 내부 처리 로직은 별도 하드웨어 블록으로 쪼개지 않고 전부 제어부 동작으로 통합 서술한다", "근거 없는 하드웨어 블록 임의 추가 금지". 위반 시 (i) 청구항에 없는 구성요소가 대표 도면과 본문의 주된 실시예로 노출되어 권리해석이 좁아지고, (ii) 부호 체계가 불필요하게 비대해지며, (iii) 실시예마다 다른 기능부 명칭이 생겨 동의어 방어망이 흐트러진다. 작성 패턴: 각 처리 문단을 `제어부(120)는 ~한다`로 시작하되 도입어를 변주(`또한`, `이어서`, `나아가`, `보다 구체적으로`)하고, 처리 구분이 필요하면 부호 없이 "문서의 구조화, 트리플의 추출과 선별, 증거 가중치의 산출과 누적, …에 이르는 일련의 처리를 수행한다" 식으로 열거한다. 자체 점검: 본문 정규식 `[가-힣]{2,8}부\(\d+\)` 결과가 청구항에 등장하는 `~부` 집합의 부분집합인지 확인.
