...
- (시드) 본 스킬 사용자의 가장 흔한 진입점은 "본문 1차 초안 완료 + 정의·변형 미적용" → **Stage 6 추천이 default 가까움**. 시그널 표시 시 이 경우를 가장 먼저 매칭 시도.
- (시드) 외부 변리사 작성본은 시그널 ⑥(금지어) 위반이 흔함 → 처음 진단 시 grep 우선 수행.
- (시드) "어느 단계?" 단발 질문일 때는 보고서를 짧게 (자료 현황표 + 추천 Stage + 권장 명령만, 위험 신호·근거 생략) 출력하여 빠르게 답.
- (2026-05-27, 사건 A 첫 사용) 시그널 ④(자체 사전 정의 단락) 검출은 표준 4단 템플릿("라 함은 ~ 의미할 수 있다") 외 변형 형식도 인정 필수. 변형 패턴: "본 명세서에서 ~ 정의될 수 있다", "본 발명에서 ~ 명명될 수 있다", "라고 함은 ~ 의미할 수 있다", "~ 라 함은 ~ 이해될 수 있다". 표준 템플릿만 검색하면 정의 적용된 명세서를 "미적용"으로 오진.
